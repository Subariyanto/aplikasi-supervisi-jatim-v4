...
# ---- 12. obs-content wrapper in renderInputObservasi ----
rep("if(tab==='supervisi')html+=renderSupervisiList();\nelse if(tab==='kokurikuler')html+=renderKokurikulerPage();\nelse if(tab==='ekstrakurikuler')html+=renderEkstrakurikulerPage();\nelse html+=renderIklimMadrasahPage();\nreturn html;",
    "html+='<div class=\"obs-content\">';\nif(tab==='supervisi')html+=renderSupervisiList();\nelse if(tab==='kokurikuler')html+=renderKokurikulerPage();\nelse if(tab==='ekstrakurikuler')html+=renderEkstrakurikulerPage();\nelse html+=renderIklimMadrasahPage();\nhtml+='</div>';\nreturn html;")
...
rep(".obs-tabs{display:flex;flex-wrap:wrap;gap:4px;border-bottom:3px solid #e2e8f0;margin-bottom:0}\n.obs-tab{padding:12px 20px;cursor:pointer;font-weight:600;color:#64748b;border-bottom:3px solid transparent;margin-bottom:-3px;transition:all .2s;white-space:nowrap}\n.obs-tab.active{color:#034f1d;border-bottom-color:#034f1d;background:#f0fdf4}",
    ".obs-tabs{display:flex;flex-wrap:wrap;gap:4px;border-bottom:3px solid #034f1d;margin-bottom:20px}\n.obs-tab{padding:12px 20px;cursor:pointer;font-weight:600;color:#64748b;border-bottom:3px solid transparent;margin-bottom:-3px;transition:all .2s;white-space:nowrap}\n.obs-tab.active{color:#034f1d;border-bottom-color:#034f1d;background:#f0fdf4}\n.obs-tab:hover{color:#034f1d}\n.obs-content{padding:0}")
...
rep("state.ekstrakurikulerIdx=data.length-1;\nnavigate('ekstrakurikuler-form');",
    "state.ekstrakurikulerIdx=data.length-1;state.activeTab='ekstrakurikuler';\nnavigate('ekstrakurikuler-form');")
